<template>
  <div style="max-width: 800px; margin: 0px auto; padding: 0px 20px">
    <div v-if="$root.user">
      <h1>
        Welcome
        <template v-if="$root.user.displayName">{{
          $root.user.displayName
        }}</template>
        <template v-else>{{ $root.user.email }}</template>
        <button style="background: none" @click="logout">
          <img style="height: 20px" src="@/assets/logout_icon_151219.png" />
        </button>
      </h1>
      ...
      <div
        ...
        v-if="($root.tokens = 1000 && $root.firstLogin == true)"
      >
        <label for="firstLogin "
          >We see its your First Login: Welcome to Progressor! We gave you 1000
          tokens for free to get started!!!</label
        >> <router-link to="/chat2">Try now!</router-link>
      </div>
 ...
 <button @click="$root.tierData = '1'">1</button> -->
      ...
      <div
        class="subscription-container"
        v-if="$root.tierData == '0'"
        style="background-color: #dadada; padding: 20px; textAlign: center, color: 'white' }"
      >
        <h1 style="font-size: 24px; margin-bottom: 10px">NO PLAN!</h1>
        <router-link
          to="/chat2"
          style="text-decoration: underline; color: rgb(0, 81, 255)"
          >If you have coins, you can still Chat</router-link
        >
      ...
      <div
        class="subscription-container"
        style="border: 7px dashed #00c4ff"
        v-else-if="$root.tierData == '1'"
        :style="{
          backgroundColor: 'rgb(44 115 205)',
          padding: '20px',
          textAlign: 'center',
          color: 'white',
        }"
      ...
      <div
        class="subscription-container"
        style="border: 7px dashed rgb(255 0 247)"
        v-else-if="$root.tierData == '2'"
        :style="{
          backgroundColor: '#6e4981',
          padding: '20px',
          textAlign: 'center',
          color: 'white',
        }"
      ...
      <div
        class="subscription-container"
        style="
          border: 16px dashed rgb(0 144 255);
          background-color: rgb(229 0 0);
          padding: 20px;
          text-align: center;
          color: white;
        "
        v-else-if="$root.tierData == '3'"
      >
        <h1 style="font-size: 24px; margin-bottom: 10px">
          Guardian Cyber Elite
        </h1>
        <button
          @click="$router.push('/manage')"
          style="
