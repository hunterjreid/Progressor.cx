<template>
  <div style="max-width: 800px; margin: 0 auto; padding: 20px">
    <h1 class="titlew">
      You're just <b>one step</b> away from becoming a member of the Progressors
      Club!
    </h1>
    <p>
      You have chosen the ${{ $route.query.pricing }} plan. Would you like to
    ...
    <li>
      <input
        ...
        @click="
          (this.customAmount = $route.query.pricing),
            (subscriptionPlan = 'month')
        "
        value="monthly"
      /><a class="subscriptionPlan">
        Monthly (${{ $route.query.pricing }}/month)</a
      >
    </li>
    <li>
      <input
        ...
        name="subscriptionPlan"
        @click="
          (this.customAmount = $route.query.pricing * 11.2),
            (subscriptionPlan = 'year')
        "
        value="annual"
      />
      <a class="subscriptionPlan"
        >Annual (${{ ($route.query.pricing * 11.2).toFixed(2) }}/year)</a
      >
      <a style="background: rgb(228, 9, 9); color: white"
        >⭐ 7% CHEAPER LESS MONTHLY</a
      >
    </li>
