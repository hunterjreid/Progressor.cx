  ...
  <div class="mainer">
    ...
    <div class="button-scroller">
      ...
      <button
        ...
        "
        v-for="(button, index) in cybersecurityButtons"
        :key="index"
        class="msg"
    ...
    <div class="dm-box" ref="dmBox">
      <div
        v-if="progressorThinking"
        class="message-bubble received thinking-bubble"
      >
    ...
    <div class="chat-bar" style="overflow: hidden">
      ...
      >
        <button
          class="bot_btn"
          style="background-color: #777b7e52; margin: 0px"
          @click="clearChat"
        >
          Clear Chat
        </button>
        ...
        <div>
          ...
          <button
            style="background-color: #777b7e52"
            class="bot_btn"
            @click="generateReport"
          >
            Generate Report
          </button>
          <button class="bot_btn" style="margin: 0px" @click="sendMessage">
            Send
          </button>
        </div>
      </div>
      <span style="color: white" v-if="messageCost !== undefined"
        >COST: {{ messageCost }} tokens</span
      >
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      conversationHistory: [
        {
          content:
            "Welcome to Progressor CX! 🚀 Hello! How can I assist you today?",
          type: "received",
          id: 0,
        },
      ],
  ...
  methods: {
    ...
    async sendMessage() {
      ...
      if (
        this.messageCost > this.$root.userTokens &&
        this.$root.tokens_bypass == false
      ) {
      ...
      this.conversationHistory.unshift({
        ...
        type: "sent",
        id: this.conversationHistory.length,
      });
        ...
        setDoc(userDocRef, tokensData)
          .then(() => {
            ...
            addDoc(usageRef, usageData)
              .then((docRef) => {
                console.log(
                  "Usage information uploaded to Firebase with ID:",
                  docRef.id
                );
                this.inputMessage = "";
              })
      ...
      try {
        ...
        const requestData = {
          prompt: {
            context:
              "I'm Progressor.cx, a cybersecurity chatbot called Progressor.cx. Originally created for incident response, but I can do a lot of cyber stuff and read code. Created by Hunter Reid from Yoobee Colleges in Auckland, NZ",
        ...
        if (candidates != undefined) {
          this.conversationHistory.unshift({
            content: this.formatMessage(candidates[0].content),
            type: "received",
            id: this.conversationHistory.length,
          });
        } else {
          this.conversationHistory.unshift({
            content:
              "I'm Progressor.cx. Feel free to ask me anything about cybersecurity, and I'll provide you with an answer!",
            type: "received",
            id: this.conversationHistory.length,
          });
        }
