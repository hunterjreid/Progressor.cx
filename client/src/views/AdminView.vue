  ...
  <div>
    <div v-if="isAdmin">
      ...
      <div style="max-width: 800px; margin: 0 auto; padding: 20px">
        ...
        <button
          style="color: rgb(255, 255, 255); background-color: red"
          @click="confirmLogout"
        >
          Logout of ADMIN
        </button>
        ...
        <p>
          ...
          providing amazing services to our customers.
        </p>
      ...
      <div style="max-width: 800px; margin: 0 auto; padding: 20px">
        <button
          style="
            ...
            background-color: rgb(23, 133, 13);
            font-size: 24px;
          "
          @click="showAddBlogPopup = true"
        >
          Add Blog +
        </button>
      ...
      <div style="max-width: 800px; margin: 0 auto; padding: 20px">
        ...
        <ul>
          <li v-for="blog in blogs" :key="blog.id">
            ...
            <button
              style="background-color: rgb(0, 88, 88)"
      ...
      <div v-if="showAddBlogPopup" class="popup">
        ...
        <textarea
          ...
          style="width: 90%; height: 256px"
        ></textarea>
        <br />
        <label>URL: https://progressor.cx/blog/</label>
        <input type="text" v-model="newBlog.url" />
        <button style="background: green" @click="addBlog">Create Blog</button>
        <br />
      ...
      <div v-if="showEditBlogPopup" class="popup">
        ...
        <button
          style="background: rgb(175, 63, 63)"
          @click="showDeleteConfirmation(selectedBlog)"
        >
          Delete this blog
        </button>
        ...
        <ul class="contact-list">
          <transition-group name="slide">
            <li v-for="(request, index) in contactRequests" :key="request.id">
              <div
                class="contact-item"
                :class="{ 'slide-out': request.deleted }"
              >
                ...
                <div class="contact-details">
                  ...
                  <img
                    ...
                    :src="`${this.$root.api_prefix}/api/get_uploaded_image/${request.picture}`"
                    style="max-width: 200px; max-height: 200px"
                    alt="Profile Picture"
                  />
                  <a v-else>No Picture</a>
                  ...
                  <button
                    style="background: rgb(182, 37, 37)"
                    @click="showConfirmationDialog(request.id)"
                  >
                    X
                  </button>
                </div>
  ...
  methods: {
    getImageUrl(base64Data) {
      ...
      const base64String = btoa(
        String.fromCharCode(...new Uint8Array(base64Data))
      );
    ...
    confirmLogout() {
      const confirmation = confirm(
        "Click OK to Confirm you want to delete this session as admin (Yo will have to login agian)?"
      );
      if (confirmation) {
        this.$router.push("/");
    ...
    saveEditedBlog() {
      ...
      axios
        .put(
          `${this.$root.api_prefix}/api/blogs/${this.selectedBlog.id}`,
          this.selectedBlog
        )
        .then((response) => {
    ...
    getPictureUrl(pictureData) {
      ...
      const base64String = btoa(
        String.fromCharCode(...new Uint8Array(pictureData))
      );
      const mimeType = "image/jpeg"; // You may need to adjust the MIME type based on your image format
      return `data:${mimeType};base64,${base64String}`;
    },
    ...
    showConfirmationDialog(requestId) {
      if (
        window.confirm(
          "Are you sure you want to delete? You may not get this back."
        )
      ) {
    ...
    markAsDeleted(requestId) {
      axios
        .put(`${this.$root.api_prefix}/api/contact-requests/${requestId}`)
        .then(() => {
          // Find the contact request in the local array and set 'deleted' to true
          const requestToUpdate = this.contactRequests.find(
    ...
    fetchContactRequests() {
      ...
      axios
        .get(`${this.$root.api_prefix}/api/contact-requests`) // Replace '/api/contact-requests' with your server endpoint
        .then((response) => {
