<template>
  <section class="my-8">
    <form class="contact-form">
      ...
      <button
        style="width: 100%; font-size: 1.24em"
        type="button"
        @click="submitForm"
      >
        Apply Now to Progressor.cx Plus+
      </button>
    </form>
  </section>
</template>
...
export default {
  ...
  methods: {
    async submitForm() {
      try {
        ...
        await axios.post(
          `${this.$root.api_prefix}/api/submit_contact`,
          formData,
          {
            headers: {
  ...
  onFileChange(event) {
    ...
    axios
      .post(`${this.$root.api_prefix}/api/upload_picture`, formData)
      .then((response) => {
